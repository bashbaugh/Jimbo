...
class Macros(commands.Cog, name='Macros'):
    ...
    async def desc_macros(self, ctx):
        """- What are macros?"""
        await ctx.send("Macros are commands that you can use to make the me say something specific, without typing it out yourself. (For example this is a macro.) There are also macros that listen for you to say certain things and then respond. You can list the macros with the `listMacros` command.")
    ...
    async def suggest(self, ctx, *, suggestion):
        """<text> - send a suggestion (text) for a macro, command, or listener."""
        user = await self.bot.fetch_user(cfg.BOT_MANAGER_ID)
        message = "{} sent suggestion: {}".format(ctx.message.author, suggestion)  
    ...
    async def on_message(self, msg):
        ...
        if msg.content.startswith(cfg.MACRO_PREFIX):
            await msg.channel.send("I don't know that macro.")
